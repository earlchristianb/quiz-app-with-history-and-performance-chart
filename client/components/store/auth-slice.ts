...
export const userLogin = createAsyncThunk(
	"auth/userLogin",
	async (details: logInCredentials, dispatch) => {
...
export const updatePicture = createAsyncThunk(
	"auth/updatePicture",
	async (image: Object, dispatch) => {
		const { data } = await API.patch("/user/image", image);
		return data;
	}
);
...
const authSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder.addCase(userLogin.pending, (state: RootStateOrAny, action) => {
			...
			builder.addCase(
				updatePicture.pending,
				(state: RootStateOrAny, action) => {
					state.status = "loading";
				}
			),
			builder.addCase(
				updatePicture.fulfilled,
				(state: RootStateOrAny, action) => {
					state.user = action.payload.userDetails;
					sessionStorage.setItem(
						"user",
