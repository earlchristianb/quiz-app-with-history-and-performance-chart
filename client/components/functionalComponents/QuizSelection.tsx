...
import React, { FC, FormEvent, FormEventHandler, useState,useRef } from "react"
...
const difficultyOptions = [
 ...
 const QuizSelection:FC=()=> {
   ...
   const handleSubmit = async(e: React.FormEvent<HTMLFormElement>) => {
     ...
     dispatch(selectActions.setSelection({
  ...
  return (
      <form
          className="w-60 md:w-80 p-8 h-60 md:h-72 space-y-2 md:space-y-3 bg-textLight drop-shadow-md shadow-2xl relative"
        ...
        <label htmlFor="difficulty" className="font-medium">Difficulty Level</label>
          ...
          <div className="flex justify-center items-center">
               <button disabled={category1===undefined && difficulty1===undefined?true:false} type="submit" className={`${category1===undefined && difficulty1===undefined?"bg-textLight":"bg-sky-400"} text-slate-700 font-bold rounded-lg p-2 mt-1 w-3/4 shadow-md shadow-primaryLight`}>
              Generate Quiz
          </button>
          </div>
